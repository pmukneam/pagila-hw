...
$$
SELECT category.name, count(*) AS sum FROM film JOIN film_category USING (film_id) JOIN category USING (category_ID) JOIN language USING (language_id) WHERE language.name = $1 GROUP BY category.name;
$$
LANGUAGE SQL
IMMUTABLE
